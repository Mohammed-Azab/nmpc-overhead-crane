...
% Music
adir  = fullfile(fileparts(fileparts(fileparts(mfilename('fullpath')))), 'assets');
songs = [dir(fullfile(adir,'*.mp3')); dir(fullfile(adir,'*.wav')); dir(fullfile(adir,'*.m4a'))];
if ~isempty(songs), musicFile = fullfile(adir, songs(1).name); else, musicFile = ''; end

plotCraneResults(res, ...
    'Title', 'Crane demo: open-loop swing', ...
    'Animate', true, ...
    'MusicFile', musicFile, ...       % your song from assets/ (empty -> silent)
    'Muted', music_muted, ...         % start muted (config in output_config.m)
    'CableLength', 2, ...
    'Speed', 1, ...
